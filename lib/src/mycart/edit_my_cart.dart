...
import '../bill print/edit_order_bill_print.dart';

class MyCartEdit extends StatefulWidget {
  ...
  @override
  _MyCartEditState createState() => _MyCartEditState();
}

class _MyCartEditState extends State<MyCartEdit> {
  ...
  void initState() {
    ...
    Getapi().getOrderforedit(context, orderId).then((value) {
      print(orderId);
      setState(() {
        previousorderitems = value;
        print(value);
      });
    });
  }
  ...
  Widget build(BuildContext context) {
     double currentOrderTotalPrice = selectedProducts.fold<double>(
      0,
      (total, item) => total + (item.quantity * item.price),
    );

    // Calculate the total price of previous orders
    double previousOrdersTotalPrice = previousorderitems.fold<double>(
      0,
      (previousTotal, item) => previousTotal + (item.quantity * item.price),
    );

    // Calculate the combined total price of previous and current orders
    double totalPrice = previousOrdersTotalPrice + currentOrderTotalPrice;
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            //     ListView.builder(
            //       shrinkWrap: true,
            //       itemCount: previousorderitems.length,
            //       itemBuilder: (context, index) {
            //         var order = previousorderitems[index];
            //         return Row(
            //           mainAxisAlignment: MainAxisAlignment.spaceBetween,
            //           children: [
            //             Text(
            ...
            //           return ListTile(
            //             title: Text(
            //               previousorderitems[index].product,
            //               style: GoogleFonts.poppins(
            //                 fontSize: 16,
            //                 fontWeight: FontWeight.w500,
            //                 color: Colors.grey[700],
            ...
            //             ),
            //             subtitle: Text(
            //               'Quantity: ${previousorderitems[index].quantity.toString()}    Price: Rs.${previousorderitems[index].price.toStringAsFixed(2)}',
            //               style: GoogleFonts.poppins(
            //                 fontSize: 14,
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Padding(
                    padding: const EdgeInsets.only(top: 20, left: 70, right: 40),
                    child: Text(
                      'Orders Details:',
                      style: GoogleFonts.poppins(
                        fontSize: 16,
                  ...
                  SizedBox(height: 10),
                  if (previousorderitems.isNotEmpty)
                    Column(
                      children: previousorderitems.map((item) {
                        return Padding(
                  ...
                  //       ),
                  //       Text(
                  //         'Rs.${previousOrdersTotalPrice.toStringAsFixed(2)}',
                  //         style: GoogleFonts.poppins(
                  //           fontSize: 16,
                  //           fontWeight: FontWeight.w600,
                  //           color: Colors.grey,
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      // }
                      var order_id =
                          await Postapi().editplaceOrder(widget.orderId!, body);
                      if (order_id != 0) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Order has been sent!'),
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => EditOrderBillPrint(
